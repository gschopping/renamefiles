...
#		my @files = glob("\"" . $filter. "\"");
		...
		foreach my $file (sort @files) {
			...
			if ($self->isnummeringdatum) {
				$newfile = My::EenBestand->new("bestand" => $file, "exifdatum" => $self->{exifdatum}, "exifdatumformaat" => $self->{exifdatumformaat},
					"datumpatroon" => $self->{patroon}, "exiftitel" => $self->{exiftitel}, "voorloop" => $self->{voorloop}, 
					"overschrijfvoorloop" => $self->{overschrijfvoorloop});
			} else {
				$newfile = My::EenBestand->new("bestand" => $file, "exifdatum" => $self->{exifdatum}, "exifdatumformaat" => $self->{exifdatumformaat},
					"datumpatroon" => $self->{patroon}, "exiftitel" => $self->{exiftitel}, "teller" => $teller, "posities" => $self->{posities},
					"voorloop" => $self->{voorloop}, "overschrijfvoorloop" => $self->{overschrijfvoorloop});
			}
